...
class ProductListUsecaseImpl @Inject constructor(private val repository: ProductListRepository, private val mapper: ProductListMapper):ProductListUsecase {
    override suspend fun fetchProductListData() : List<ProductListCompact>? =
        repository.getProductListFromDb()?.map { data-> mapper.convertProductListLocalToCompact(data)!! }
}